// Uno: each round, the winner scores 0 and everyone else adds up the point value of
// the cards left in their hand — lowest total wins (first to a target score, commonly
// 500, loses). Demonstrates two things Flip7 doesn't: ascending sort order via
// getSortDirection, and a digit-entry keypad instead of preset increment buttons,
// since a single round's points can be any value.
//
// Run with: bun run scripts/seed_uno_template.ts

import { upsertTemplate } from './lib/seed_helpers';

const SCRIPT = `
function appendDigit(ctx, digit) {
  if (ctx.cell) ctx.cell.n1 = ctx.cell.n1 * 10 + digit;
}
...
function onPress_backspace(ctx) {
  if (ctx.cell) ctx.cell.n1 = Math.floor(ctx.cell.n1 / 10);
}
...
async function main() {
  await upsertTemplate(
    'Uno',
    'Uno scoring: each round the winner scores 0, everyone else adds up the cards left in hand. Lowest total wins.',
    SCRIPT,
    [{ name: 'primary', starts: true, columns: 4, rows: 4, buttons }]
  );
}
